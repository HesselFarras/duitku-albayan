...
<div class="flex flex-col items-start p-[48px] gap-[48px] w-full max-w-[800px] min-h-screen mx-auto bg-[#F9F9F8]">
    ...
    <div class="w-full space-y-2">
        <p class="font-[Inter] font-bold text-[10px] tracking-[0.2em] uppercase text-[#887364]">
            AGENDA / <span class="text-[#8D4B00]">TAMBAH KEGIATAN BARU</span>
        </p>
    ...
    <form action="{{ route('activities.store') }}" method="POST" class="w-full bg-white rounded-[40px] p-10 border border-[#DBC2B0]/10 shadow-sm space-y-6 font-[Manrope]">
        @csrf

        <div class="space-y-2">
            <label class="text-xs font-black uppercase text-[#887364] tracking-wider">Nama Agenda / Kegiatan</label>
            <input type="text" name="title" required class="w-full rounded-xl border-[#DBC2B0]/40 focus:border-[#8D4B00] focus:ring-[#8D4B00] text-sm" placeholder="Contoh: Santunan Anak Yatim Pekanan">
        </div>

        <div class="grid grid-cols-2 gap-4">
            <div class="space-y-2">
                <label class="text-xs font-black uppercase text-[#887364] tracking-wider">Tanggal</label>
                <input type="date" name="date" required class="w-full rounded-xl border-[#DBC2B0]/40 focus:border-[#8D4B00] focus:ring-[#8D4B00] text-sm">
            </div>
            <div class="space-y-2">
                <label class="text-xs font-black uppercase text-[#887364] tracking-wider">Waktu / Jam</label>
                <input type="time" name="time" required class="w-full rounded-xl border-[#DBC2B0]/40 focus:border-[#8D4B00] focus:ring-[#8D4B00] text-sm">
            </div>
        </div>

        <div class="space-y-2">
            <label class="text-xs font-black uppercase text-[#887364] tracking-wider">Lokasi Pelaksanaan</label>
            <input type="text" name="location" required class="w-full rounded-xl border-[#DBC2B0]/40 focus:border-[#8D4B00] focus:ring-[#8D4B00] text-sm" placeholder="Contoh: Ruang Utama Lantai 1">
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div class="space-y-2">
                ...
                <select name="funding_source" required class="w-full rounded-xl border-[#DBC2B0]/40 focus:border-[#8D4B00] focus:ring-[#8D4B00] text-sm cursor-pointer">
                    <option value="Kas Utama">Kas Utama Masjid</option>
                    <option value="Zakat">Dana Zakat Mal/Fitrah</option>
                    <option value="Infaq Terikat">Infaq Khusus / Donatur</option>
                    <option value="Dana Sosial">Kas Sosial & Anak Yatim</option>
                </select>
            </div>
            <div class="space-y-2">
                <label class="text-xs font-black uppercase text-[#887364] tracking-wider">Anggaran Biaya (Rp)</label>
                <input type="number" name="budget" required min="0" class="w-full rounded-xl border-[#DBC2B0]/40 focus:border-[#8D4B00] focus:ring-[#8D4B00] text-sm" placeholder="0">
            </div>
        </div>
        ...
        <div class="space-y-2">
            ...
            <textarea name="description" rows="4" class="w-full rounded-xl border-[#DBC2B0]/40 focus:border-[#8D4B00] focus:ring-[#8D4B00] text-sm" placeholder="Tuliskan detail singkat atau info penting pelaksanaan acara di sini..."></textarea>
        </div>
        ...
        <div class="flex items-center justify-end gap-3 pt-4">
            ...
            <button type="submit" class="px-6 py-3 bg-[#8D4B00] hover:bg-amber-900 text-white font-bold rounded-2xl text-xs shadow-sm transition-all">Simpan Agenda</button>
        </div>
    </form>
</div>
@endsection